...
class About extends React.Component {
  ...
  render() {
    ...
    if (this.props.resources.greetingParams) {
      greeting = (
        <h3>
          {this.props.resources.greetingParams.greeting}
          { }
          {this.props.resources.greetingParams.name}
        </h3>
      );
    } else {
    ...
    return (
      <div className={css.root}>
        ...
        <form onSubmit={this.handleSubmit}>
          <label className={css.label} htmlFor={`g${dk}`}>
            Greeting:
            { }
            <input className={css.textInput} id={`g${dk}`} type="text" />
          </label>
          <label className={css.label} htmlFor={`n${dk}`}>
            Person:
            { }
            <input className={css.textInput} id={`n${dk}`} type="text" />
          </label>
